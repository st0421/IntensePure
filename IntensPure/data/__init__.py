...
def define_dataset(logger, opt):
    ...
    dataset_opt = opt['datasets'][opt['phase']]['which_dataset'] # datasets -> train -> which_dataset
    ...
    dataloder_opt = opt['datasets'][opt['phase']]['dataloader'] # datasets -> train -> dataloader
    ...
    if valid_split > 0.0 or 'debug' in opt['name']: 
        if isinstance(valid_split, int): 
            ...
            valid_len = valid_split # valid_len = 2
        else: 
