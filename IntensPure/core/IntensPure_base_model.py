...
class BaseModel():
    ...
    def denormalize(self, normalized_image, mean, std) :
    ...
    def train(self):
        while self.epoch <= self.opt['train']['n_epoch'] and self.iter <= self.opt['train']['n_iter']:
            ...
            train_log, y_noisy, y_cond, path = self.train_step()
            ...
            if self.epoch % self.opt['train']['val_epoch'] == 0: # Validation
                self.logger.info("\n\n\n------------------------------Validation Start------------------------------")
                if self.val_loader is None:
                    self.logger.warning('Validation stop where dataloader is None, Skip it.')
                else:
                    val_log = self.val_step(y_noisy, y_cond, path)
    ...
    @abstractmethod
    def val_step(self, y_noisy, y_cond):
